...
public class Window extends JFrame implements View {
	...
	Button srcMMEcoreButton, trgMMEcoreButton, atlTransButton, outputFileButton, smButton;
	...
	public Window() {
		...
		smButton = new Button("Get Similarity Matrix");
		Font newButtonFont= new Font("DIALOG",Font.BOLD,12);
		smButton.setFont(newButtonFont);
		...
		panelbl8.add(smButton, BorderLayout.NORTH);
		...
		this.setTitle("Similarity Matrix Calculator");
		this.pack();
...
//		this.setResizable(false);
	...
	public void controller(ActionListener controller) {
		...
		smButton.addActionListener(controller);
		smButton.setActionCommand(GET_SM);		
	}
	...
	public void getSMClick() {
			...
			String pathCSVFile = outputFileTField.getText();
			...
			boolean ok = checkFields(pathATLTransformation, pathSrcEcoreMMFile,
					pathTrgEcoreMMFile, pathCSVFile);
			...
			if (ok){
				...
				try {
					...
					SimilarityMatrix sm = new SimilarityMatrix("tempTypesExtracted.xmi", pathCSVFile + "/" + 
					...
					File file = new File("tempModel.xmi");
					File file2 = new File("tempTypesExtracted.xmi");
					file.delete();
					file2.delete();
				
					//For throwing the excel created
					File f = new File(pathCSVFile + "/" + 
							ATLTransName.substring(0, ATLTransName.lastIndexOf('.')) + "SM.csv");
					...
					try {
						if(f.exists()==true) { 
							...
							messageLabel.append("Error while opening .csv file created"); 
							messageLabel.update(messageLabel.getGraphics());
						} 
					} 
						catch (IOException e) { 
						} 
					if (sm.getMean() <= 0.15 && sm.getDeviation()<= 0.15){
						messageLabel2.setForeground(Color.blue);
						messageLabel2.setText("It is advisable to apply our approach on this model transformation!! \n");
					} else {
						messageLabel2.setForeground(Color.red);
						messageLabel2.setText("\n \n \n It not is advisable to apply our approach on this model transformation \n");
					}
				} catch (ATLCoreException e) {
					messageLabel.setForeground(Color.red);
					messageLabel.append("ATLCoreException: " + e.toString());
